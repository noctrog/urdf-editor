...
std::string resolveFilePath(const std::string& mesh_filename, const std::string& urdf_dir) {
    ...
    if (mesh_filename.rfind(package_prefix, 0) == 0) {
        ...
        // Resolve URIs that refer to the package containing the URDF without
        // requiring a ROS environment.
        ...
        // ROS_PACKAGE_PATH entries can be package roots (as generated by
        // catkin) or directories containing packages (for example an install
        // space's share directory).
        ...
        if (ros_package_path) {
            ...
            while (std::getline(paths, entry, ':')) {
                ...
                fs::path root(entry);
                if (is_requested_package(root)) {
                    return (root / relative_path).string();
                }

                fs::path child = root / package_name;
                if (is_requested_package(child)) {
                    return (child / relative_path).string();
                }
            }
        }
